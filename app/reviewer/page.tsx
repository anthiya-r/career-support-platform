...
export default function ReviewerPage() {
	...
	const loadAssignments = useCallback(async () => {
		...
		const payload = (await res.json().catch(() => null)) as {
			submissions?: typeof assignments;
			error?: string;
		} | null;
		if (!res.ok) {
			...
			<main className="mx-auto grid max-w-5xl gap-6 px-4 py-8 md:grid-cols-2">
				<section className="space-y-4">
					...
					{assignments.map((item) => (
						...
						>
							<p className="text-base font-semibold text-[#43302E]">
								{item.title}
							</p>
							<p className="text-sm text-slate-600">
								{item.applicant} · {item.status}
							</p>
							<button
								type="button"
								onClick={() => setSelectedId(item.id)}
								className="mt-3 rounded-md border border-slate-300 px-3 py-2 text-sm font-medium text-[#43302E] transition hover:bg-slate-100"
							>
								Review
							</button>
						</div>
				...
				<section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
					<h2 className="text-lg font-semibold text-[#43302E]">Review Form</h2>
					...
					{selectedSubmission?.resumeFileBase64 &&
