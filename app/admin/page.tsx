...
export default function AdminPage() {
	...
	const [pendingSubmissions, setPendingSubmissions] = useState<
		PendingSubmission[]
	>([]);
	const [selectedReviewerBySubmission, setSelectedReviewerBySubmission] =
		...
		<div className="min-h-screen">
			...
			<main className="mx-auto grid max-w-6xl gap-6 px-4 py-8 lg:grid-cols-2">
				<section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
					<h1 className="text-2xl font-bold text-[#43302E]">Admin</h1>
					<p className="mt-1 text-sm text-slate-600">
						Promote reviewers and assign resume submissions.
					</p>
					...
					<form onSubmit={handlePromote} className="mt-5 space-y-2">
						...
						<div className="flex flex-col gap-2 sm:flex-row">
							...
							<button
								type="submit"
								disabled={loading}
								className="rounded-md bg-[#3A8FC1] px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:bg-slate-300"
							>
								Promote
							</button>
						</div>
					...
					<div className="mt-5">
						<h2 className="text-sm font-semibold text-[#43302E]">
							Current reviewers
						</h2>
						<ul className="mt-2 space-y-2 text-sm text-slate-700">
							...
							{reviewers.length === 0 ? (
								<li className="text-slate-500">No reviewers yet.</li>
							) : null}
						</ul>
					</div>
				</section>
				...
				<section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
					<h2 className="text-lg font-semibold text-[#43302E]">
						Assign reviewer
					</h2>
					<div className="mt-4 space-y-3">
						{pendingSubmissions.map((item) => (
							...
							>
								<p className="font-semibold text-[#43302E]">{item.title}</p>
						...
						{pendingSubmissions.length === 0 ? (
							<p className="text-sm text-slate-500">
								No pending or assigned submissions right now.
							</p>
						) : null}
					</div>
				</section>
